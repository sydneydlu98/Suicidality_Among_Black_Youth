...
# subgroup analysis (only study those who had severe suicide attempt among those who had suicide attempt)
...
```{r}
summary(svyglm(QN26 ~ QN25+QN24+QN66+QN23+QN41+QN2, data = black_data, design = yrbsdes, family = quasibinomial()))
...
```{r}
summary(svyglm(QN27 ~ QN25+QN23+QN24+QN2, data = black_data, design = yrbsdes, family = quasibinomial()))
...
```{r}
summary(svyglm(QN28 ~ QN23+QN25+QN1+QN49+QN2, data = black_data, design = yrbsdes, family = quasibinomial()))
...
df_29 <- data.frame(variable = c(1:5, 8:25, 30:64, 66:99), value = rbind(df1, df2, df3, df4))
df_29 %>%
  arrange(value)
```

```{r}
summary(svyglm(QN29 ~ QN69+QN50+QN52+QN2, data = black_data, design = yrbsdes, family = quasibinomial()))
